        ...
        while j < max_epLength:
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps % (update_freq) == 0:
                    ...
                    los = sess.run(mainQN.loss,feed_dict={mainQN.scalarInput:np.vstack(trainBatch[:,0]),mainQN.targetQ:targetQ, mainQN.actions:trainBatch[:,1]})
                    loss.append(los)
                    jList.append(total_steps)
                    updateTarget(targetOps,sess)
            rAll += r
            s = s1
            env.renderEnv()
